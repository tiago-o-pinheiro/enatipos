...
export const CompatibilityView = ({ a, b }: CompatibilityViewProps) => {
  ...
  const bName = t(`mbti.type.${b}.name` as TranslationKey)

  const sections = SECTIONS.map((section) => {
    const key = `compatibility.mbti.${pair}.${section}` as TranslationKey
    const value = t(key)
      ...
      <section className='animate-fade-in'>
        ...
        <div className='mb-6 flex items-center justify-between gap-3'>
          <div className='flex flex-1 flex-col items-center gap-2'>
            ...
            />
            <span className='font-display text-[clamp(28px,8vw,40px)] font-light leading-none tracking-[-0.04em] text-gold-400'>
              {a}
            </span>
        ...
        <div className='mt-10 flex flex-col gap-3'>
          ...
          >
            {t('compatibility.back.mbti', { code: a })}
          </Link>
